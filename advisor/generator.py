...
Each scenario is a dict:
{
  "name": str,
  "description": str,
  "overloaded_node": str,     # the node being evacuated from, per CONTRACT.md's
                               # RecommendRequest shape — candidates' latency_ms
                               # is keyed by this
...
# Shared by every scenario below — the real system carries this as
# RecommendRequest.overloaded_node (see /shared/schemas/recommend_request.schema.json);
# these synthetic scenarios don't need it to be a "real" candidate, just a
# stable key for each candidate's latency_ms map.
OVERLOADED_NODE = "regA-c1-overloaded"
...
def _candidate(
    ...
    timestamp = datetime.fromtimestamp(now - age_seconds, tz=timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    return {
        "node_id": node_id,
        "timestamp": timestamp,
        "cpu_percent": cpu,
        "mem_percent": mem,
        "latency_ms": {OVERLOADED_NODE: latency},
        "trust_score": trust,
        "history_load_avg_5m": hist_load,
    }
...
def generate_scenarios() -> List[Dict[str, Any]]:
    ...
    # 7. Mixed: two stale + one valid -> the valid one wins by default.
    # (Previously modeled "unavailable" candidates via a fictional `available`
    # flag that doesn't exist in models.py, scorer.py, or the real shared
    # schema, so it never actually disqualified anything. Staleness is a real
    # disqualification path, so this now tests the same "mixed
    # disqualifications" idea for real instead of a no-op flag.)
    scenarios.append({
        "name": "mixed_disqualifications",
        "description": "Two stale candidates, one healthy valid candidate.",
        "overloaded_node": OVERLOADED_NODE,
        "candidates": [
            _candidate("edge16", cpu=15, mem=15, latency=15, trust=0.95, hist_load=15, age_seconds=200, now=now),
            _candidate("edge17", cpu=20, mem=20, latency=20, trust=0.9, hist_load=20, age_seconds=150, now=now),
    ...
    # 8. Memory-constrained candidate loses to CPU-constrained candidate with better memory.
    scenarios.append({
    ...
    # 9. Historical load matters even when instantaneous CPU looks fine (thrashy node).
    scenarios.append({
    ...
    # 10. Three-way race with a clear best-of-three.
    scenarios.append({
        "name": "three_way_race",
    ...
    # 11. Extremely overloaded cluster: pick "least bad" option.
    scenarios.append({
    ...
    # 12. Zero-trust candidate should almost never win even if resources are great.
    scenarios.append({
    ...
    # 13. Single candidate, trivially wins (sanity check for the single-node edge case).
    scenarios.append({
    ...
    # 14. Empty candidate list -> no winner (guards against upstream bugs).
    scenarios.append({
    ...
    # 15. Boundary staleness: exactly at the threshold should still count as stale (> not >=).
    scenarios.append({
